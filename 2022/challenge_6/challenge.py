...
first time a marker could occur is after the fourth character is received, making the most recent four characters mjqj. Because j is
...
with open("challenge_6/input.txt") as text_file:
    buffer = text_file.read()


def part_1(buffer=str):
    for i in range(4, len(buffer)):
        s = set(buffer[(i-4):i])
        if len(s) == 4:
            return "First marker after character " + str(i)


def part_2(buffer=str):

...
your help finding the errors. Every item type is identified by a single lowercase or uppercase letter (that is, a
...
def split_list(item_list: list):
    ...
    return item_list[:half], item_list[half:]
...
def get_items(items: list) -> int:
    ...
    for item in items:
        compartments_a, compartments_b = split_list(item) 
        item_type = list(set(compartments_a).intersection(compartments_b))

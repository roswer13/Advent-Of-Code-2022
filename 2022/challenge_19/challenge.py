...
robots. Fortunately, there is an abundant amount of clay nearby that you can use to make them waterproof.

In order to harvest the clay, you'll need special-purpose clay-collecting robots. To make any 
type of robot, you'll need ore, which is also plentiful but in the opposite direction from the clay.
...
(Blueprints have been line-wrapped here for legibility. The robot factory's actual assortment 
...
Using blueprint 1 in the example above, the largest number of geodes you could open in 24 minutes 
is 9. One way to achieve that is:
...
first blueprint has ID 1 and can open 9 geodes, so its quality level is 9. The second blueprint has 
ID 2 and can open 12 geodes, so its quality level is 24. Finally, if you add up the quality levels of 
...
However, blueprint 2 from the example above is still better; using it, the largest number of 
geodes you could open in 32 minutes is 62.

You no longer have enough blueprints to worry about quality levels. Instead, for each of the 
first three blueprints, determine the largest number of geodes you could open; then, multiply 
these three values together.

Don't worry about quality levels; instead, just determine the largest number of geodes you could 
open using each of the first three blueprints. What do you get if you multiply these numbers together?
...
def T(blueprint, mins):
    ...
    mo = max(cc, cb1, cg1)
    Q, maxx = collections.deque(), 0

    def p(g, t, o, c, b, ro, rc, rb):
        nonlocal maxx
        if t <= 0 or g+t*t-t < maxx:
            return
        maxx = max(maxx, g)
        Q.append((g, t, o, c, b, ro, rc, rb))

    p(0, mins, 0, 0, 0, 1, 0, 0)
    while Q:
        g, t, o, c, b, ro, rc, rb = Q.pop()
        if g+t*t-t < maxx:
            continue
        if rb:
            dt = max(res(cg1, o, ro), res(cg2, b, rb))
            p(g + max(t-dt, 0), t-dt, o+dt*ro-cg1,
              c+dt*rc, b+dt*rb-cg2, ro, rc, rb)
        if rc and b+(rb*t) < cg2*t:
...
with open("2022/challenge_19/input.txt") as f:
    lines = [tuple(map(int, re.findall('\\d+', x)))
             for x in f.read().splitlines()]
    total1 = sum(x[0] * T(x, 24) for x in lines)
    total2 = functools.reduce(lambda a, b: a*b, (T(x, 32)
                              for x in lines[:3]), 1)
    print(total1, total2)

...
You know they're headed to the grove where the star fruit grows, so if you can figure out 
...
numbers after the value 0, wrapping around the list as necessary. In the above example, the 1000th 
...
811589153, the 2000th is 2434767459, and the 3000th is -1623178306; adding these together produces 1623178306.
...
with open("2022/challenge_20/input.txt") as f:
    numbers = [int(l) for l in f.read().splitlines()]
# print(numbers)

# tuples of (order, value)
c_list = list(enumerate(numbers))
# print(c_list)
...
for i in range(c_len):
    for j in range(c_len):
        ...
        order, c = x = c_list[j]
        if order == i:
            # new position = j+c  = where to place before
            # after removing initial element
            # new element will be placed in a shorter list of size  len-1
            # starting with j again and moving by c
            # that's why mod len-1 !!
            n_j = (j+c) % (c_len-1)
            del c_list[j]
            c_list.insert(n_j, x)
            #print("move", j, "->", n_j, c_list)
            break
# print(c_list)
...
# tuples of (order, value)
c_list = [(i, v*DEC_KEY) for i, v in enumerate(numbers)]
# print(c_list)

...
You and the elephants finally reach fresh air. You've emerged near the base of a large 
volcano that seems to be actively erupting! Fortunately, the lava seems to be flowing away 
from you and toward the ocean.
...
obsidian! The cooling rate should be based on the surface area of the lava droplets, so you 
take a quick scan of a droplet as it flies past you (your puzzle input).

Because of how quickly the lava is moving, the scan isn't very good; its resolution is quite 
low and, as a result, it approximates the shape of the lava droplet with 1x1x1 cubes on a 3D 
...
body = set(tuple(map(int, e.split(",")))
           for e in open("2022/challenge_18/input.txt", "rt"))


def neighbors(x, y, z): return ((x-1, y, z), (x+1, y, z),
                                (x, y-1, z), (x, y+1, z), (x, y, z-1), (x, y, z+1))


p1 = len(body)*6 - sum((p, q, r) in body for x, y,
                       z in body for p, q, r in neighbors(x, y, z))
...
p2 = sum((p, q, r) in outside for x, y,
         z in body for p, q, r in neighbors(x, y, z))
